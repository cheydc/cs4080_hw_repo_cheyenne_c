  ...
  for (;;) {
    ...
    switch (instruction = READ_BYTE()) {
      ...
      case OP_GET_GLOBAL: {
        ...
        if (vm.globalNames[idx] == NULL) { runtimeError("Undefined variable."); return INTERPRET_RUNTIME_ERROR; }
        push(vm.globals[idx]);
        break;
      }
      case OP_DEFINE_GLOBAL: { uint8_t idx = READ_BYTE(); vm.globals[idx] = pop(); break; }
      case OP_SET_GLOBAL: {
        uint8_t idx = READ_BYTE();
        if (vm.globalNames[idx] == NULL) { runtimeError("Undefined variable."); return INTERPRET_RUNTIME_ERROR; }
